...
//
// KNOWN LIMITATION: ветка onePlaced может давать до 29 наложений карточек
// (|x1-x2| < CW на одном y) между onePlaced-задачами и уже размещёнными
// extern-позициями (children-block, соседние семьи). Три независимые
// попытки фикса (глобальный row-floor, точечная pre-sweep проверка,
// anchor-check внутри sequential loop) устраняли все 29 пересечений, но
// каждая вызывала каскадную регрессию 24+ новых больших разрывов (gap>300)
// в других, ранее корректных парах — структурное свойство однонаправленного
// prevRight-sweep, не баг конкретной реализации. Требует алгоритма
// компакции по кластерам (union-find/graph-based), а не точечного патча.
// Принято как известное ограничение — см. также TODO_data_issues.md.
...
// ── layoutAllComponents ──────────────────────────────────────────────────
// Объединяет ВСЕ компоненты связности (findConnectedComponents) в одну
// общую раскладку целого дерева, размещая их последовательно слева направо
// с горизонтальным отступом между несвязанными ветвями.
//
// ВАЖНО про startX внутри layoutComponentV3: этот параметр там сейчас НЕ
// используется для сдвига координат (используется только в вычислении
// width = maxX - startX внутри самой layoutComponentV3), позиции внутри
// неё всегда считаются от x≈0 независимо от переданного startX. Поскольку
// layoutComponentV3/placeFamiliesInRow/placeChildrenOfFamiliesInRow —
// протестированный и закрытый вопрос (см. шапку файла), сдвиг компонент
// сделан здесь, снаружи: для каждой компоненты вызываем layoutComponentV3
// с startX=0, получаем "локальные" координаты, затем сами вычисляем
// смещение (offset = желаемый startX - минимальный x локальной раскладки)
// и прибавляем его при копировании в allPositions. layoutComponentV3 не
// модифицируется.
//
// COMPONENT_GAP = SIBLING_GAP * 5 = 110: внутри компоненты соседние "чужие"
// подсемьи и так разводятся на SIBLING_GAP (22) / COUPLE_GAP (12) — отступ
// в 5×SIBLING_GAP визуально однозначно читается как "это уже другая,
// структурно не связанная ветвь семьи", но не создаёт неоправданно большого
// пустого поля с учётом того, что среди 41 компоненты много состоят всего
// из 1-3 персон.
function layoutAllComponents(IDX) {
  const rows = assignRows(IDX);
  const components = findConnectedComponents(IDX);
  // сортировка по убыванию размера — самая большая компонента первая,
  // для предсказуемости и удобства чтения результата (не влияет на
  // корректность раскладки)
  const sortedComponents = components.slice().sort((a, b) => b.length - a.length);

  const COMPONENT_GAP = SIBLING_GAP * 5; // = 110
  ...
  sortedComponents.forEach((component, componentIndex) => {
    const { positions: localPositions } = layoutComponentV3(IDX, component, rows, 0);

    const xs = Object.values(localPositions).map(p => p.x);
    const minX = xs.length ? Math.min(...xs) : 0;
    const maxX = xs.length ? Math.max(...xs) : 0;
    const width = xs.length ? (maxX + CW - minX) : 0;

    const startX = cursorX;
    const offset = startX - minX;

    for (const [id, pos] of Object.entries(localPositions)) {
      allPositions[id] = { x: pos.x + offset, y: pos.y };
    }

    const endX = startX + width;
    componentBoundaries.push({ componentIndex, startX, endX, size: component.length });

    cursorX = endX + COMPONENT_GAP;
  });

  // убираем лишний COMPONENT_GAP, добавленный после последней компоненты
  const totalWidth = sortedComponents.length ? (cursorX - COMPONENT_GAP) : 0;

  return { allPositions, totalWidth, componentBoundaries };
}
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    ...
    findConnectedComponents, assignRows, layoutAllComponents
  };
}
...
if (typeof require !== 'undefined' && require.main === module) {
  ...
  // ── layoutAllComponents: все 41 компоненты вместе ──────────────────────
  console.log('');
  console.log('══════════════════════════════════════════════════════════');
  console.log('=== layoutAllComponents: ВСЕ компоненты (полное дерево) ===');
  console.log('══════════════════════════════════════════════════════════');

  const allResult = layoutAllComponents(IDX);
  const allIds = Object.keys(allResult.allPositions);
  const totalPersons = Object.keys(IDX.nodes).length;
  console.log('Персон получили позиции ВСЕГО:', allIds.length, '(цель: ' + totalPersons + ' из ' + totalPersons + ')');

  if (allIds.length !== totalPersons) {
    const missing = Object.keys(IDX.nodes).filter(id => !allResult.allPositions[id]);
    console.log('НЕ ХВАТАЕТ позиций у:', missing.length, 'персон:');
    missing.forEach(id => {
      const comp = components.findIndex(c => c.includes(id));
      console.log('  ' + id + ' (компонента #' + comp + ', размер ' + (comp >= 0 ? components[comp].length : '?') + ')');
    });
  }

  // дубли координат ВО ВСЕХ 239
  const byCoordAll = new Map();
  for (const [id, pos] of Object.entries(allResult.allPositions)) {
    const key = pos.x + ',' + pos.y;
    if (!byCoordAll.has(key)) byCoordAll.set(key, []);
    byCoordAll.get(key).push(id);
  }
  const dupCoordsAll = [...byCoordAll.entries()].filter(([, ids]) => ids.length > 1);
  console.log('Дублей координат ВО ВСЕХ ' + allIds.length + ':', dupCoordsAll.length, '(цель: 0)');
  dupCoordsAll.forEach(([key, ids]) => {
    const comps = ids.map(id => components.findIndex(c => c.includes(id)));
    console.log('  ' + key + ' -> ' + ids.map((id, i) => id + '(компонента #' + comps[i] + ')').join(', '));
  });

  console.log('');
  console.log('Общая ширина всей раскладки (totalWidth):', allResult.totalWidth.toFixed(0));

  console.log('');
  console.log('Таблица componentBoundaries (все ' + allResult.componentBoundaries.length + ' компонент):');
  console.log('  idx | size | startX  | endX    | width');
  allResult.componentBoundaries.forEach(b => {
    const w = b.endX - b.startX;
    console.log(
      '  ' + String(b.componentIndex).padStart(3) +
      ' | ' + String(b.size).padStart(4) +
      ' | ' + b.startX.toFixed(0).padStart(7) +
      ' | ' + b.endX.toFixed(0).padStart(7) +
      ' | ' + w.toFixed(0).padStart(5)
    );
  });

  console.log('');
  console.log('ИТОГ: ' + allIds.length + '/' + totalPersons + ' персон, дублей: ' + dupCoordsAll.length);
}
